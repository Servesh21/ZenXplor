...
const Profile: React.FC<{
  ...
  return (
    <div className="max-w-xl mx-auto p-8 rounded-2xl shadow-xl mt-10 bg-white text-gray-900 border border-gray-200 dark:bg-gray-800 dark:text-white dark:border-gray-700 transition-all duration-300">
      ...
      {message && (
        <div
          className={`p-4 mb-6 rounded-lg text-center transition-all duration-300 ${status === "success"
                ...
                <div className="grid grid-cols-5 gap-3">
                  {predefinedImages.map((img, index) => (
                    <div key={index} className="relative cursor-pointer group" onClick={() => setProfileImage(img)}>
                      ...
                      <div className={`rounded-full overflow-hidden transition transform ${profileImage === img ? "scale-105" : "scale-100 group-hover:scale-105"
                        }`}>
                        <img
                          src={img}
            ...
            {isEditMode && (
              <div className="flex space-x-4 pt-4">
                ...
                <button
                  onClick={handleSave}
                  disabled={isLoading}
                  className={`flex-1 py-3 px-4 rounded-lg text-white font-medium transition-all ${isLoading
